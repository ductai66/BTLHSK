...
namespace QuanLyCHDTDD
{
    public partial class HoaDon : Form
    {
        ...
        private int kiemtraNVDL()
        {
            string constr = "Data Source=DESKTOP-JQNQUR1\\SQLEXPRESS; Initial Catalog = QuanLyCHDTDD; Integrated Security = True";
            using (SqlConnection cnn = new SqlConnection(constr))
            {
                string k = textMaNV.Text;
                string sql = "Select nv.[Mã Nhân Viên] from vv_nhanvien as nv where nv.[Mã Nhân Viên] = N'" + k.ToString() + "'";
                using (SqlCommand cmd = new SqlCommand(sql, cnn))
                {
                    using (SqlDataAdapter ad = new SqlDataAdapter(cmd))
                    {
        ...
        private int kiemtraNVExit()
        {
            string constr = "Data Source=DESKTOP-JQNQUR1\\SQLEXPRESS; Initial Catalog = QuanLyCHDTDD; Integrated Security = True";
            using (SqlConnection cnn = new SqlConnection(constr))
            {
                string k = textMaNV.Text;
                string sql = "select lg.sMaNV from tblLogin as lg " +
                    "where lg.Username = '" + DangNhap.temp + "' AND lg.sMaNV = '" + k.ToString() + "'";
                using (SqlCommand cmd = new SqlCommand(sql, cnn))
                {
                    using (SqlDataAdapter ad = new SqlDataAdapter(cmd))
                    {
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection cnn = new SqlConnection(constr))
            {
                try
                {
                    ...
                    else if(kiemtraNVDL() == 0)
                    {
                        MessageBox.Show("Nhân viên này đã nghỉ");
                        textMaNV.Focus();
                    }
                    else if (kiemtraNVExit() == 0)
                    {
                        MessageBox.Show("Đây không phải mã nhân viên của bạn", "Thông báo", MessageBoxButtons.OK);
                        textMaNV.Focus();
                    }
                    else if (kiemtraKH() == 0)
                    {
        ...
        private void mNgaydat_MaskInputRejected(object sender, MaskInputRejectedEventArgs e)
        {
