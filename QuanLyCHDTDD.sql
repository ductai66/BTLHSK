...
select nv.[Mã Nhân Viên] from vv_nhanvien as nv, tblLogin as lg where nv.[Mã Nhân Viên] = lg.sMaNV and lg.Username = 'dothetai8'
...
--nhân viên đã nghỉ
Create view vv_nv_danghi
as
	select sMaNV as[Mã Nhân Viên], sTenNV as[Tên Nhân Viên], sGioitinh as[Giới tính],sDiachi as[Địa chỉ], dNgaysinh as[Ngày sinh],fLcb as[Lương cơ bản],sSdt as[Số điện thoại],sTrangthai as[Trạng thái]
	from tblNhanVien where sTrangthai =N'Nghỉ'

select * from vv_nv_danghi
...
CREATE TRIGGER tg_dambaoluonghang
ON tblChiTietHoaDon
FOR INSERT,UPDATE
AS
	BEGIN
		...
		SET @MaMH = (select sMaMH from INSERTED)
		SET @soluongban = (SELECT iSoluong from INSERTED)
		SET @soluonghienco =( SELECT tblMatHang.iSoluong from tblMatHang,INSERTED
							 WHERE tblMatHang.sMaMH = @MaMH)
		IF(@soluongban > @soluonghienco)
			BEGIN
				PRINT N'Không hợp lệ. Số lượng mua vượt quá số lượng hàng hiện có.'
...
--2.Tạo trigger tự động cập nhật số lượng hiện có của mặt hàng (trong bảng mặt hàng) sau khi đã bán

CREATE TRIGGER tg_soluongMH
ON tblChiTietHoaDon
FOR INSERT,update
AS
BEGIN
	...
	DECLARE @mamh NVARCHAR(10)
	SET @soluongban = (SELECT iSoluong FROM INSERTED)
	set @mamh = (select tblMatHang.sMaMH from INSERTED , tblMatHang WHERE INSERTED.sMaMH = tblMatHang.sMaMH )
	UPDATE tblMatHang
...
---Tạo trigger cho phép giá bán chỉ lớn hơn hoặc bằng giá hàng

CREATE TRIGGER tg_dbgiaban
ON tblChiTietHoaDon
FOR  INSERT,UPDATE
AS
BEGIN
	...
	SET @giahang = (select tblMatHang.fGiaHang from tblMatHang,inserted WHERE tblMatHang.sMaMH = inserted.sMaMH)
	SET @giaban = (select fGiaban from inserted)
	if(@giaban >= @giahang)
	begin
		print N'Hợp lệ.'
	end
	...
	begin
		print N'Không hợp lệ.Giá bán chỉ lớn hơn hoặc bằng giá hàng'
		ROLLBACK TRANSACTION
	end
END

--- mỗi hóa đơn chỉ được lập tối đa 3 hóa đơn

create trigger tg_ddh
on tblChiTietHoaDon
for insert
as
	begin
		declare @somh int
		set @somh = (select Count(tblChiTietHoaDon.sMaHD) from tblChiTietHoaDon,tblHoaDon,inserted
						where inserted.sMaHD = tblHoaDon.sMaHD AND tblHoaDon.sMaHD = tblChiTietHoaDon.sMaHD
						Group by tblChiTietHoaDon.sMaHD)
		if(@somh <= 3)
			begin
				print 'ok'
	...
	--Khi xóa chitiethoadon thì cập nhật lại số lượng mặt hàng hiện có
create trigger tg_chitiet_mh
on tblChiTietHoaDon
after delete
as
begin
declare @slb int
declare @mamh nvarchar(10)
set @mamh = (select tblMatHang.sMaMH from deleted , tblMatHang WHERE deleted.sMaMH = tblMatHang.sMaMH )
set @slb = (select iSoluong from deleted)
update tblMatHang
set iSoluong = iSoLuong+@slb
where sMaMH = @mamh
end

drop trigger tg_chitiet_mh

delete from tblChiTietHoaDon
where sMaHD = N'HD01' AND sMaMH = N'MH02'
